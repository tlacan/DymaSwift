...
struct MyTripsScreen: View {
  ...
  init(engine: Engine) {
    _viewModel = State(initialValue: MyTripsScreenViewModel(engine: engine))
  }
    ...
    NavigationStack {
      GeometryReader { geo in
        ScrollView(.vertical) {
          ...
          .refreshable {
            viewModel.loadTrips(showLoader: false)
          }
      }
    }.task {
      viewModel.loadTrips(showLoader: true)
    }
  }
  ...
  func tripImage(_ trip: TripModel, withoutUnavailableText: Bool = false) -> some View {
    if let city = viewModel.city(for: trip) {
      AsyncImage(url: URL(string: city.image), content: { image in
    ...
    VStack(alignment: .leading) {
      ...
      if viewModel.pastTrips.isEmpty {
        R.string.localizable.myTripsPastSectionEmpty()
          .textView(style: .description, multiligneAlignment: .leading)
          .frame(maxWidth: .infinity, alignment: .leading)
      }
  ...
  func pastItem(_ trip: TripModel, fullWidth: CGFloat) -> some View {
    ...
    return ZStack(alignment: .bottom) {
      ...
      VStack(alignment: .leading) {
        VStack(alignment: .leading) {
          viewModel.city(for: trip)?.name.textView(style: .sectionTitle, lineLimit: 1)
...
#Preview {
  MyTripsScreen(engine: Engine(mock: true))
}
